...
**2026-04-20 increment — Session 2026-04-20 contract-drift repair (Q1–Q8)**. Triggered by the 2026-04-20 merge of `main` into `001-wolof-translate-mobile`, which changed `web_server.py` to populate a server-rendered English `audio_url` for `wolof_to_english` completions (macOS `say -o ...`). Left unchecked the mobile client would auto-download and play the server m4a via `expo-audio`, bypassing FR-004's on-device `expo-speech` requirement and wasting bandwidth on a file nothing consumes. Session 2026-04-20 locked 8 decisions (`spec.md` §Clarifications Session 2026-04-20, Q1–Q8); the net impact on this plan is a **client-only + contract-doc-only** repair. Affected files: `src/state/pipeline-store.ts` (direction guard at `:150`, plus a download-failure suffix in the completion branch for Q6), `src/state/__tests__/pipeline-store.test.ts:175` and `src/audio/__tests__/player.test.ts:69` (fixture updates to the new `english_audio` + populated-`audio_url` wire shape, plus a new Q6 download-failure test), and `contracts/bff-api.md` §2 line 152-155 + §6 line 292 (rewritten to "BFF MAY populate; client MUST ignore per FR-004"). No BFF change (Q5 accepts the wasted render; Q8 keeps the 409 branch as defensive). FR-004 is amended in the spec; no new FR is added. No new dependency, no schema change, no UI mock needed — the only user-visible delta is a "(failed to download audio)" suffix appended to an existing text surface for the Q6 english_to_wolof fallback.
...
- [x] **IV. Comment Traceability** — **PASS**. Any comments added in implementation will carry the `(001-wolof-translate-mobile:<task-id>)` suffix per the convention. The plan itself is a design artifact, so no such suffixes appear in plan prose. **2026-04-20 increment**: the Q4 guard edit at `src/state/pipeline-store.ts:150` and the Q6 completion-branch edit both add new comment(s) carrying the new Phase 9 task-id suffixes allocated in `tasks.md`; no comment is reused from the T070/T075c pool.
...
## Session 2026-04-20 Design Detail (contract-drift repair)

### Motivation

The 2026-04-20 merge of `main` into `001-wolof-translate-mobile` changed `web_server.py` to render a server-side English m4a for `wolof_to_english` completions (via macOS `say -o ...`) and populate `result.audio_url` with a download path. Left unchecked the mobile `drainPoll` completion branch at `src/state/pipeline-store.ts:150` would call `client.downloadAudio(requestId)` unconditionally when `audioUrl` is truthy, then pass the downloaded file to `defaultPlayer.playResult`. For `wolof_to_english` that silently replaces the FR-004-mandated on-device `expo-speech` rendering with `expo-audio` playback of a server-rendered English voice — a contract drift against FR-004. Session 2026-04-20 locked Q1 on "always on-device; ignore `audio_url` for `wolof_to_english`" and amended FR-004 to make that explicit. Q2 documents the drift in `contracts/bff-api.md` so the two surfaces' actual behavior is recorded without either side needing a coordinated change; Q4 places the guard in the client (pipeline-store) per separation-of-concerns; Q6 closes a second, unrelated drift — the `english_to_wolof` download-failure fallback currently falls through to `expo-speech.speak(translatedText, 'en-US')`, which would mispronounce Wolof with an English voice; Q7 refreshes two stale test fixtures; Q5 and Q8 record deliberate BFF inaction.

### Code-change map

| Touch point | Edit | Expected diff |
|---|---|---|
| `mobile-app/src/state/pipeline-store.ts:150` (Q4) | Replace `const localAudioUri = state.result.audioUrl ? await client.downloadAudio(requestId) : null;` with a conditional that also short-circuits on `state.result.direction === 'wolof_to_english'`. New comment carries the Phase 9 task-id suffix. | +3 / -1 lines |
| `mobile-app/src/state/pipeline-store.ts` completion branch (Q6) | After the `const localAudioUri = …` line, if `terminal.direction === 'english_to_wolof'` AND `localAudioUri === null`, append `" (failed to download audio)"` to `terminal.translatedText` BEFORE `persistToHistory(terminal)`. Skip the `void defaultPlayer.playResult(terminal, …)` call. Dispatch `{ type: 'playbackStarted' }` then immediately `{ type: 'playbackEnded' }` so the phase cycles `completed → playing → completed` (the state-machine at `src/pipeline/state-machine.ts:224-227` guards `playbackStarted` with `if (state.phase !== 'completed') return state;`, so the first dispatch is a legal transition; `playbackEnded` is a no-op outside `playing`, so a harmless second dispatch matches the Q6 "skipped playing" note). History insert still gets `audioPath: ''` via the existing empty-`audioUrl` branch of `persistToHistory`. | +8 / -1 lines |
| `mobile-app/src/api/bff-client.ts` (`downloadAudioImpl`, `:418`) | **No change required.** `downloadAudio` already returns `null` uniformly for both 4xx responses (`if (res.status >= 400) return null;`) and thrown exceptions (the `catch { return null; }` at the bottom of the try block). This satisfies the C12 contract test and Q6's "returns null" precondition without caller-side disambiguation. | 0 lines |
| `mobile-app/src/state/__tests__/pipeline-store.test.ts:175` (Q7) | Update the `wolof_to_english` text-only fixture from `{ audioUrl: null, outputMode: 'text_only' }` to the post-merge wire shape `{ audioUrl: '/api/requests/req-1/audio', outputMode: 'english_audio' }`. Keep every existing assertion (`mockDownloadAudio` NOT called, `mockHistoryInsert` with `audioPath: ''`, transient unlink, `mockPlayResult` called once with the TTS-fallback-ready terminal) — Q4's guard keeps `localAudioUri = null` regardless of the new `audioUrl`, so the branch under test is still TTS-only. Add a **new adjacent test** for Q4 (asserts `mockDownloadAudio.mock.calls.length === 0` even when `audioUrl` is populated) and a **new adjacent test** for Q6 (mocks `english_to_wolof` + `mockDownloadAudio.mockResolvedValue(null)`, asserts (a) no `playResult` invocation with a file, (b) history insert with `audioPath: ''`, (c) `translatedText` contains `"(failed to download audio)"`). | +2 / -2 lines for the fixture update; +2 new tests ~40 lines each |
| `mobile-app/src/audio/__tests__/player.test.ts:69` (Q7) | Update the `wolof_to_english` fixture `baseTextOnly` (or its equivalent) from `{ audioUrl: null, outputMode: 'text_only' }` to `{ audioUrl: '/api/…/audio', outputMode: 'english_audio', localAudioUri: null }`. The branch under test (`createAudioPlayer` NOT called, `speakText` called with `en-US`) is unchanged: the player keys off `localAudioUri`, which Q4's caller-side skip keeps at `null`. No new assertions here — the fixture modernization is enough. | +2 / -2 lines |
| `mobile-app/specs/001-wolof-translate-mobile/contracts/bff-api.md` §2 (line 152-155) and §6 (line 292) (Q2) | Rewrite the two blockquote passages from "`audio_url` is populated on every `english_to_wolof` completion response as of FR-039 … It is `null` for `wolof_to_english` completions (on-device TTS per FR-004)" to "The BFF MAY populate `audio_url` on both directions as of the 2026-04-20 merge. The mobile client MUST ignore `audio_url` for `wolof_to_english` per FR-004 (on-device TTS); `wolof_to_english` `audio_url` values — when populated — MUST NOT be downloaded, persisted, or played.". §6 line 292 gets a parallel rewrite noting that `wolof_to_english` `audio_url` is now "server-rendered but client-ignored". | ~8 lines rewritten across two anchors |

### Test surface (Phase 9 — TDD order)

| Test name | Purpose |
|---|---|
| `pipeline-store.test.ts :: wolof_to_english with populated audio_url skips download (Q4)` | Assert `mockDownloadAudio` is NOT called when a `wolof_to_english` completion arrives with `outputMode: 'english_audio'` + `audioUrl: '/api/.../audio'`. Assert `mockPlayResult` is still invoked (TTS branch) and history insert has `audioPath: ''`. |
| `pipeline-store.test.ts :: english_to_wolof download failure appends suffix and skips playback (Q6)` | Mock `mockDownloadAudio.mockResolvedValue(null)` on an `english_to_wolof` completion. Assert `mockPlayResult` NOT invoked with a local file, history insert has `audioPath: ''`, `translatedText` ends in `(failed to download audio)`, and the phase returns to `completed`. |
| `pipeline-store.test.ts :: wolof_to_english fixture wire shape (Q7)` (updated in-place) | Existing test at line 175 keeps all assertions; the fixture is modernized to `outputMode: 'english_audio'` + populated `audioUrl`. The test's purpose — TTS-only path for `wolof_to_english` — is unchanged. |
| `player.test.ts :: baseTextOnly fixture wire shape (Q7)` (updated in-place) | Existing test at line 69 keeps all assertions; the fixture is modernized to the new wire shape. Same rationale. |

### Risk surface

| Risk | Mitigation |
|---|---|
| Post-merge BFF now spends ~100 ms + a few KB of disk per `wolof_to_english` completion rendering an English m4a that no mobile client consumes. | **Accepted per Q5.** Leave `web_server.py` symmetric across directions; revisit only if server disk or latency becomes a concern. |
| The Q6 `"(failed to download audio)"` suffix is English-only, bypassing the FR-035 i18n catalog. | v1 UI is English-only per `spec.md` §Clarifications Session 2026-04-16 (Q: UI localization); the suffix lands as a literal in the Q6 completion-branch edit. When locale additions arrive in a future spec, the suffix is promoted to a `translation.failure.download_failed` key in `src/i18n/locales/en/messages.po` and the edit at `pipeline-store.ts` switches to `t\`…\``. No refactor-by-refactor risk in v1. |
| The Q4 guard's placement in `pipeline-store.ts` duplicates intent already present in `player.ts` (which also keys off `localAudioUri`). | **Intentional.** Q4 chose pipeline-store placement specifically to also skip the `downloadAudio` network round-trip, not just the playback decision. The player's `localAudioUri`-keyed branch is the defensive backstop. |

### Post-Design Re-Check — Session 2026-04-20

After the design above:

- **Principles I, II, III, IV, V, VI, VII, VIII — all PASS.** No new deviation introduced; no new dependency; no new UI mock surface; no schema change; no BFF contract change (documenting drift is not redefining contract — Q2). Principle II's TDD order is preserved by landing the Q4 guard test and Q6 download-failure test in `pipeline-store.test.ts` BEFORE the implementation edits in `pipeline-store.ts`. Principle IV's comment-traceability suffixes are assigned via the Phase 9 task-ids allocated in `tasks.md`. Principle VI's single-commit-per-phase convention is honored by landing every 2026-04-20 change under one `Phase9-Session2026-04-20` commit.
- **FR-003a Back-end scope gate** — not applicable (the Session 2026-04-20 increment proposes zero BFF change; Q5 and Q8 are explicit "no BFF change").
- **Downstream follow-up (owned by `/speckit-tasks`)**: a new Phase 9 — Session 2026-04-20 contract-drift repair (Q1–Q8) lands in `tasks.md` with test-first ordering (guard test + download-failure test + fixture update tasks → implementation → contract doc edit → verification → commit). No mock-first pair (no UI surface). No BFF task (Q5/Q8 reject BFF changes).
