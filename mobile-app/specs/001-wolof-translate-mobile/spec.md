...
Triggered by the 2026-04-20 merge of `main` into `001-wolof-translate-mobile`, which changed `web_server.py` to return a populated `audio_url` for `wolof_to_english` completions (server-rendered English m4a via macOS `say -o ...`). The 8-impact survey walks each consequence and locks a decision.

- Q1: FR-004 drift — the backend now returns a server-rendered English m4a for `wolof_to_english`; mobile client would auto-play the file via `expo-audio` instead of `expo-speech`. → A: **Always on-device (ignore audio_url).** Client MUST use `expo-speech` for `wolof_to_english` regardless of whether the BFF populates `audio_url`. The server m4a for this direction becomes unused by the mobile client. FR-004 amended accordingly.
- Q2: Contract doc drift — `contracts/bff-api.md` §2 (line 152-155) and §6 (line 292) assert `audio_url` is null for `wolof_to_english`. → A: **Document "may be populated; client ignores".** Update `contracts/bff-api.md` to reflect that the BFF MAY populate `audio_url` for `wolof_to_english` and that the mobile client MUST ignore it per FR-004. Both surfaces' actual behavior is recorded; neither side needs coordinated change.
- Q3: English voice selection — with Q1 locking `wolof_to_english` to on-device TTS, the English voice is whatever `expo-speech` resolves for `en-US`. → A: **Use iOS default voice.** No pinning, no Settings picker in v1. `Speech.speak(text, { language: 'en-US' })` keeps resolving to the user's system default. No code change, no spec change beyond Q1.
- Q4: Where to enforce "ignore audio_url for `wolof_to_english`" in the client. → A: **Guard in pipeline-store.** `src/state/pipeline-store.ts:150` MUST skip `downloadAudio(...)` when `direction === 'wolof_to_english'`, regardless of whether the wire payload populates `audio_url`. Unit test MUST assert `mockDownloadAudio` is NOT called when a `wolof_to_english` completion arrives with a populated `audio_url`. Downstream the history entry retains the empty-`audioPath` sentinel and replay uses `expo-speech` on the stored `translatedText`.
- Q5: BFF-side cost of the now-unused `wolof_to_english` m4a render. → A: **Accept the wasted render.** Leave `web_server.py` symmetric across directions; the extra ~100 ms and a few KB of disk are negligible. No BFF follow-up, no retention policy in v1. Revisit only if server disk or latency becomes a concern.
- Q6: `english_to_wolof` download-failure fallback. The current code falls through to `expo-speech.speak(translatedText, 'en-US')`, which would mispronounce Wolof with an English voice. → A: **Show text-only, no speech fallback; append a user-visible note.** When `downloadAudio` returns null for an `english_to_wolof` completion, the client MUST NOT invoke `expo-speech` and MUST NOT attempt any audio playback. Append "(failed to download audio)" to the displayed translated text so the user understands why nothing is playing. Pipeline transitions completed → (skipped playing) → completed. History still persists the entry with an empty `audioPath` sentinel so no stale/partial file lingers.
- Q7: Stale test coverage. `src/state/__tests__/pipeline-store.test.ts:175` and `src/audio/__tests__/player.test.ts:69` use `outputMode: 'text_only'` + `audioUrl: null` for `wolof_to_english`; the real BFF now emits `outputMode: 'english_audio'` + populated `audio_url`. → A: **Update tests to match the new wire contract.** Fixtures MUST emit `outputMode: 'english_audio'` + `audioUrl: '/api/.../audio'` for `wolof_to_english` completions. The pipeline-store test MUST assert (a) `downloadAudio` is NOT called (Q4 guard), (b) `expo-speech` is invoked on the `translatedText`, (c) history insert carries `audioPath: ''`. The player test MUST assert the same TTS-only behavior under the new wire shape. New fixture also adds coverage for the Q6 `english_to_wolof` download-failure path: when `downloadAudio` returns null, assert no `expo-speech` call and the translated-text string gets the "(failed to download audio)" suffix.
- Q8: BFF 409 branch reachability. `/api/requests/{id}/audio` returns 409 when `output_mode` is not in `{wolof_audio, english_audio}` or `audio_url` is missing; post-merge this branch is practically unreachable for `wolof_to_english` in production. → A: **Leave the 409 branch.** Keep the guard as defensive coding — it still fires if the render path fails. No change to `web_server.py`, no change to `tests/test_audio_endpoint.py::test_audio_endpoint_409_wrong_output_mode`. Accept the dead-in-prod-but-defensive branch; the cost is near zero and the safety net is preserved.
...
- **FR-004**: On successful completion, the app MUST display the
  ...
  - For Wolof → English: a spoken rendition of the English translation
    produced on-device (system text-to-speech). The client MUST use
    on-device TTS for this direction **even when the BFF populates
    `result.audio_url`** — the server-rendered English m4a is ignored and
    MUST NOT be downloaded, persisted, or played. (Decided 2026-04-20 in
    response to the main→001 merge that began emitting `audio_url` for
    `wolof_to_english`.)
