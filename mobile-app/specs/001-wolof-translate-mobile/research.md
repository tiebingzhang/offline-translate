...
4. **Audio result**: the BFF exposes `GET /api/requests/{id}/audio` and sets `result.audio_url = "/api/requests/{id}/audio"` on every `english_to_wolof` completion (FR-039, folded in-session 2026-04-17). The mobile client consumes `audio_url` from the result and downloads the AAC/m4a via `expo-file-system.downloadAsync`. The `wolof_to_english` direction leaves `audio_url = null` and the client falls back to on-device `expo-speech` English TTS (FR-004). **BE-1 is no longer a pending prerequisite.**
...
| # | Risk | Mitigation |
|---|---|---|
| R-A | ~~BE-1 (`audio_url` + `/audio` endpoint) is a hard prerequisite for US1 closure.~~ **RESOLVED 2026-04-17** via FR-039 (in-session BFF fold-in). See `plan.md` §FR-039 and tasks T144–T151. |
